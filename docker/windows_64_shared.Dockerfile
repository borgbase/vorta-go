...
RUN apt-get -qq update && apt-get --no-install-recommends -qq -y install ca-certificates git apt-utils
...
ENV QT_INSTALLER qt-unified-linux-x64-online.run
RUN curl -sL --retry 10 --retry-delay 60 -O https://download.qt.io/official_releases/online_installers/$QT_INSTALLER && chmod +x $QT_INSTALLER
RUN QT_QPA_PLATFORM=minimal ./$QT_INSTALLER --no-force-installations --script $GOPATH/src/github.com/therecipe/qt/internal/ci/iscript.qs LINUX=true
